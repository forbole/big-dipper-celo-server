...
} from '../../api/validators/validators';
import Contracts from '../../api/contracts/contracts';

Blocks.rawCollection().createIndex({
  number: -1,
}, {
  unique: true,
});
...
Accounts.rawCollection().createIndex({
  address: 1,
}, {
  unique: true,
});
...
ValidatorGroups.rawCollection().createIndex({
  address: 1,
}, {
  unique: true,
});
...
Contracts.rawCollection().createIndex({
  address: 1,
}, {
  unique: true,
});
